...
In the next section we describe the Bass \citep{bass69}  diffusion model, followed by the results of our analysis of fitting the Bass diffusion model on the Gentoo portage package dependency graph. 
...
\emph{Innovators}: Some individuals in the market that do not yet use the innovation, might decide to adopt the innovation. The rate at which this happens is $p$, the coefficient of innovation. The number of user that do not use the innovation is $M-A$, so the inflow of adopters is $p(M-A)$.
